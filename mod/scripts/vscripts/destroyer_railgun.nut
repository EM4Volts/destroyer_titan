global function Destroyer_MpTitanWeaponStunLaser_Init

global function Destroyer_OnWeaponAttemptOffhandSwitch_titanweapon_stun_laser
global function Destroyer_OnWeaponPrimaryAttack_titanweapon_stun_laser

#if SERVER
global function Destroyer_OnWeaponNPCPrimaryAttack_titanweapon_stun_laser
global function Destroyer_AddStunLaserHealCallback
#endif

const FX_EMP_BODY_HUMAN			= $"P_emp_body_human"
const FX_EMP_BODY_TITAN			= $"P_emp_body_titan"
const Destroyer_FX_SHIELD_GAIN_SCREEN		= $"P_xo_shield_up"
const Destroyer_SHIELD_BODY_FX			= $"P_xo_armor_body_CP"
...
void function Destroyer_MpTitanWeaponStunLaser_Init()
{

	PrecacheParticleSystem( Destroyer_FX_SHIELD_GAIN_SCREEN )
	PrecacheParticleSystem( Destroyer_SHIELD_BODY_FX )
...
bool function Destroyer_OnWeaponAttemptOffhandSwitch_titanweapon_stun_laser( entity weapon )
{
...
var function Destroyer_OnWeaponPrimaryAttack_titanweapon_stun_laser( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	#if CLIENT
...
#if SERVER
var function Destroyer_OnWeaponNPCPrimaryAttack_titanweapon_stun_laser( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	return Destroyer_OnWeaponPrimaryAttack_titanweapon_stun_laser( weapon, attackParams )
}
...
void function StunLaser_DamagedTarget( entity target, var damageInfo )
{
	...
	if ( attacker.GetTeam() == target.GetTeam() )
	{
		...
		if ( target.IsTitan() && IsValid( attackerSoul ) && hasEnergyTransfer )
		{
			...
			{
				int shieldbodyFX = GetParticleSystemIndex( Destroyer_SHIELD_BODY_FX )
				int attachID
				if ( target.IsTitan() )
...
void function Destroyer_AddStunLaserHealCallback( void functionref(entity,entity,int) func )
{
...
void function Vanguard_ShieldGain( entity attacker, var eventVal )
{
	if ( attacker.IsPlayer() )
	{
		...
		if ( IsValid( cockpit ) )
			StartParticleEffectOnEntity( cockpit, GetParticleSystemIndex( Destroyer_FX_SHIELD_GAIN_SCREEN	), FX_PATTACH_ABSORIGIN_FOLLOW, -1 )
